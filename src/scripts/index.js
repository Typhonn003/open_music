...
function cardTemplate(item) {
  ...
  let valueBRL = new Intl.NumberFormat("pt-BR", {
    style: "currency",
    currency: "BRL",
  }).format(price);
...
function filterList(list) {
  ...
  rangeInput.addEventListener("input", () => {
    let listRangeFilter = list.filter(
      (product) => rangeInput.value >= product.price
    );
  ...
  genreButtons.forEach((itemButton) => {
    itemButton.addEventListener("click", () => {
      ...
      let listFilter = list.filter((product) =>
        product.category.includes(itemCategory)
      );
